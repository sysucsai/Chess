...
class Move(object):
    ...
    def to_chinese(self):
        ...
        man_side_int = {ChessSide.RED:0,ChessSide.BLACK:1}[man_side]
        ...
        if man_species in [ PieceT.KING,  PieceT.ROOK,  PieceT.CANNON,  PieceT.PAWN]:
                if diff == 0 : 
                        ...
                        dest_str = v_change_index[man_side_int][diff]
                else :
                        dest_str = v_change_index[man_side_int][-diff]
        else : #士相马的规则
                dest_str = h_level_index[man_side_int][self.p_to.x]
    ...
    def __get_chinese_name(self, p_from):
        ...
        if man_species in [ PieceT.KING,  PieceT.ADVISOR,  PieceT.BISHOP]:
                return man_name + h_level_index[man_side_int][p_from.x]
        ...
        if man_species in [ PieceT.ROOK,  PieceT.CANNON,  PieceT.KNIGHT,  PieceT.PAWN]:
                ...
                if count == 1:
                        ...
                        return pos_name2[man_side_int][pos_index] + man_name
                elif count == 3:
                        #TODO 查找另一个多子行
                        return pos_name3[man_side_int][pos_index] + man_name
                elif count == 4:
                        return pos_name4[man_side_int][pos_index] + man_name
                elif count == 5:
                        return pos_name5[man_side_int][pos_index] + man_name
