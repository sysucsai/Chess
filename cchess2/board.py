...
#-----------------------------------------------------#
FULL_INIT_FEN = 'rnbakabnr//1c5c1/p1p1p1p1p///P1P1P1P1P/1C5C1//RNBAKABNR  - - 0 1'
...
class BaseChessBoard(object) :
    ...
    def is_valid_move(self, pos_from, pos_to):
        ...
        if self.move_side and (from_side != self.move_side) :
            print(self.move_side,from_side)
            return False
...
class ChessBoard(BaseChessBoard):
        ...
        for piece in defenders :
            for move_it in piece.create_moves():
                if self.is_valid_move_t(move_it):
                        if self.is_checked_move(move_it[0], move_it[1]) == 0:
                                return False
        return True
    ...
    def get_king(self, side):
        limit_y = ((0,1,2), (7,8,9))
        for x in (3,4,5):
                for y in limit_y[{ChessSide.RED:0, ChessSide.BLACK:1}[side]]:
                        fench = self._board[y][x]
                        if not fench :
                                continue
                        if fench.lower() == 'k':
...
if __name__ == '__main__':
        ...
        print(board.copy().move(Pos(7,2),Pos(4,2)).to_chinese() == '炮二平五')
        print(board.copy().move(Pos(1,2),Pos(1,1)).to_chinese() == '炮八退一')
        print(board.copy().move(Pos(7,2),Pos(7,6)).to_chinese() == '炮二进四')
        print(board.copy().move(Pos(7,7),Pos(4,7)).to_chinese() == '炮８平５')
        print(board.copy().move(Pos(7,7),Pos(7,3)).to_chinese() == '炮８进４')
        print(board.copy().move(Pos(6,3),Pos(6,4)).to_chinese() == '兵三进一')
        print(board.copy().move(Pos(8,0),Pos(8,1)).to_chinese() == '车一进一')
        print(board.copy().move(Pos(0,9),Pos(0,8)).to_chinese() == '车１进１')
        print(board.copy().move(Pos(4,0),Pos(4,1)).to_chinese() == '帅五进一')
        print(board.copy().move(Pos(4,9),Pos(4,8)).to_chinese() == '将５进１')
        print(board.copy().move(Pos(2,0),Pos(4,2)).to_chinese() == '相七进五')
        print(board.copy().move(Pos(5,0),Pos(4,1)).to_chinese() == '仕四进五')
        print(board.copy().move(Pos(7,0),Pos(6,2)).to_chinese() == '马二进三')
