...
class King(Piece):
    ...
    def create_moves(self):
        ...
        return list(filter(self.board.is_valid_move_t, moves))
...
class Advisor(Piece): 
    ...
    def is_valid_pos(self, pos):
         return True if pos() in advisor_pos[{ChessSide.RED:0,ChessSide.BLACK:1}[self.side]] else False
...
class Bishop(Piece): 
    def is_valid_pos(self, pos):
        return True if pos() in bishop_pos[{ChessSide.RED:0,ChessSide.BLACK:1}[self.side]] else False
...
class Pawn(Piece): 
    ...
    def is_valid_move(self, pos):
        ...
        if (not over_river) and (step == not_over_river_step[{ChessSide.RED:0,ChessSide.BLACK:1}[self.side]]):
                return True
        
        if over_river and (step in over_river_step[{ChessSide.RED:0,ChessSide.BLACK:1}[self.side]]):
                return True
